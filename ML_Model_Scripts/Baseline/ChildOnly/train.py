...
def load_stance_data(path):
    """Load stance data - filter for child_only and extract text from premise"""
    with open(path, 'r', encoding='utf-8') as f:
        lines = [json.loads(line.strip()) for line in f]
    
    # Filter for child_only entries
    child_only_lines = [line for line in lines if line['input_type'] == 'child_only']
    
    # Extract text from premise (remove "Context: " prefix)
    processed_lines = []
    for line in child_only_lines:
...
def tokenize_child_only(example):
...
def run_experiment(run_id):
    ...
    # Tokenization
    dataset = dataset.map(tokenize_child_only, batched=True)
    dataset = dataset.rename_column("stance", "labels")  # Assuming stance is already 0,1,2
...
def main():
    print("Starting Child-Only Stance Detection Experiments")
    ...
    with open('child_only_results.json', 'w') as f:
        json.dump(results_summary, f, indent=2)
    
    print(f"\nResults saved to child_only_results.json")
